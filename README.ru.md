...
- Спарклайн недавнего тренда по каждому окну и прогноз исчерпания: если при текущем темпе окно упрётся в 100 % раньше сброса, строка показывает когда.
...
Каждая строка держит короткую локальную историю своего процента (в `UserDefaults`, без токенов). Как накопится несколько замеров с заметным движением, рядом с процентом появляется **спарклайн** — шкала жёстко 0–100, чтобы крутизна отражала реальный темп, а не автомасштаб. По тем же замерам методом наименьших квадратов считается **прогноз исчерпания**: если при текущем темпе окно упрётся в 100 % *раньше* сброса, добавляется строка-предупреждение — `предел ≈ <время>` — жёлтым, а если до этого меньше 45 минут — красным. Плоские и падающие окна, а также те, что успевают сброситься, не показывают ничего. По наведению по-прежнему виден точный момент сброса.
...
- **Refresh-токен ротируется.** В ответе обычно приходит новый, и старый в тот же момент перестаёт работать. Поэтому результат записывается обратно в Keychain: без записи сломался бы вход самого CLI. Запись накладывается на *свежепрочитанную* запись, сохраняя `subscriptionType`, `rateLimitTier` и все поля, о которых панель не знает.
- **Право записи проверяется до запроса** — холостой записью той же самой записи. Если Keychain писать не даёт, обновление не делается вовсе: иначе ротация отозвала бы старый токен, а новый девать было бы некуда.
