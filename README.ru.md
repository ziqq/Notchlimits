...
Каждая строка держит короткую локальную историю своего процента (в `UserDefaults`, без токенов). По этим замерам методом наименьших квадратов считается **прогноз исчерпания**: если при текущем темпе окно упрётся в 100 % *раньше* сброса — это и есть важный момент, поэтому он занимает строку тайминга: `предел ≈ <время>` жёлтым, а если до этого меньше 45 минут — красным. Точное время сброса остаётся по наведению. Плоские и падающие окна, а также те, что успевают сброситься, показывают обычную строку сброса. Прогнозу нужно несколько замеров с разбросом хотя бы в пять минут, поэтому он появляется не сразу после запуска.
