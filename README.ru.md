...
Подзаголовок — `план · почта`, как у Codex. План (`Pro`, `Max`) — из записи Keychain; почты нет ни в одном эндпоинте, поэтому её берём из `claude auth status --json`: CLI запускается подпроцессом один раз на профиль (с `CLAUDE_CONFIG_DIR` для доп. профилей), результат кэшируется.
...
Приложение ходит в три официальных эндпоинта и больше никуда: два usage-эндпоинта выше плюс OAuth-эндпоинт Anthropic, когда токен доступа Claude протух (см. [Обновление токена](#обновление-токена)). Токены не логируются и не пишутся в обычные файлы; единственное, что записывается, — обновлённый токен Claude, обратно в ту же запись Keychain, которой владеет CLI. Бинарь `claude` запускается локально подпроцессом ради версии и `auth status` (почта аккаунта) — вывод остаётся на машине, и ни один токен не передаётся в аргументах команды. На диск идут только настройки и последние проценты со временем сброса — в `UserDefaults`, без единого токена. `URLSession` работает в ephemeral-режиме, без кук и дискового кэша.
