...
Контекстное меню: **Обновить** (принудительный опрос, минуя таймеры и бэкофф), **Добавить аккаунт Claude / Codex…**, подменю **Скрыть** / **Переименовать колонку**, **Показать скрытые колонки**, **Удалить аккаунт** (с подтверждением — удаляет вход и профиль аккаунта), подменю **Горячая клавиша**, **Запуск при входе** (`SMAppService`), **Проверить наличие обновлений…** (предлагает **Обновить сейчас** — установку на месте с проверкой SHA-256 — либо открывает страницу релиза) и **Выход**.
