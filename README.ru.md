...
Контекстное меню: **Обновить** (принудительный опрос, минуя таймеры и бэкофф), **Добавить аккаунт Claude / Codex…**, подменю **Скрыть** / **Переименовать колонку**, **Показать скрытые колонки**, **Удалить аккаунт** (с подтверждением — удаляет вход и профиль аккаунта), подменю **Горячая клавиша**, **Запуск при входе** (`SMAppService`), **Проверить обновления** и **Выход**.
