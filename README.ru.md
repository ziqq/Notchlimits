...
Приложение ходит в три официальных эндпоинта и больше никуда: два usage-эндпоинта выше плюс OAuth-эндпоинт Anthropic, когда токен доступа Claude протух (см. [Обновление токена](#обновление-токена)). Токены не логируются и не пишутся в обычные файлы; единственное, что записывается, — обновлённый токен Claude, обратно в ту же запись Keychain, которой владеет CLI. Бинарь `claude` запускается локально один раз ради строки `--version` (идёт в User-Agent); почта аккаунта читается из конфига CLI `~/.claude.json`, а не запрашивается. На диск идут только настройки и последние проценты со временем сброса — в `UserDefaults`, без единого токена. `URLSession` работает в ephemeral-режиме, без кук и дискового кэша.
