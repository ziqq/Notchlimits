...
*Скриншоты — на сгенерированных данных: почты, окно `Opus` и проценты условные, это не реальный аккаунт.*
...
- Прогноз исчерпания: если при текущем темпе окно упрётся в 100 % раньше сброса, строка показывает когда.
- Обновление в один клик из релизов GitHub: **Проверить наличие обновлений** скачивает новую сборку, сверяет SHA-256, подменяет бандл и перезапускается — браузер не нужен (открыть страницу релиза остаётся запасным вариантом).
...
DMG для установки перетаскиванием (приложение и ярлык на `Applications`) пакуется из собранного бандла:

```bash
./scripts/build_dmg.sh   # → build/NotchLimits-<версия>.dmg
```
...
Контекстное меню: **Обновить** (принудительный опрос, минуя таймеры и бэкофф), **Добавить аккаунт Claude / Codex…**, подменю **Скрыть** / **Переименовать колонку**, **Показать скрытые колонки**, **Удалить аккаунт** (с подтверждением — удаляет вход и профиль аккаунта), **Активный аккаунт Codex / Claude →** (сохранить текущий и переключить, какой аккаунт использует обычная команда `codex`/`claude`), подменю **Горячая клавиша**, **Запуск при входе** (`SMAppService`), **Проверить наличие обновлений…** (предлагает **Обновить сейчас** — установку на месте с проверкой SHA-256 — либо открывает страницу релиза), **О программе Notch Limits** (нативная панель с текущей версией) и **Выход**.
...
По умолчанию — **⌘P**. Это глобальный хоткей, поэтому пока Notch Limits запущен, он перехватывает «Печать» в других приложениях — и настраивается: правый клик → **Горячая клавиша**:
...
- **`re-auth: запусти claude` / `codex`** — годного токена нет и продлить его нечем: refresh-токен отсутствует или отвергнут (Claude), либо авторизации нет или она протухла (Codex).
...
Каждая строка держит короткую локальную историю своего процента (в `UserDefaults`, без токенов). По этим замерам методом наименьших квадратов считается **прогноз исчерпания**: если при текущем темпе окно упрётся в 100 % *раньше* сброса, к строке сброса дописывается `· предел ≈ <время>` жёлтым, а если до этого меньше 45 минут — красным. Обратный отсчёт до сброса при этом виден всегда, а точная дата предела — по наведению. Плоские и падающие окна, а также те, что успевают сброситься, показывают просто строку сброса. Прогнозу нужно несколько замеров с разбросом хотя бы в пять минут, поэтому он появляется не сразу после запуска.
...
Кроме мониторинга нескольких аккаунтов колонками, панель умеет переключать, какой аккаунт использует обычная команда `codex` / `claude` — как [codex-account-switcher](https://github.com/liuzhao1225/codex-account-switcher), но для обоих провайдеров и прямо из челки. Правый клик → **Активный аккаунт Codex →** / **Активный аккаунт Claude →**:

- **Сохранить текущий** кладёт активный аккаунт в локальную библиотеку.
- Выбор сохранённого (галочка на активном) спрашивает подтверждение и делает его активным. Текущий всегда сохраняется первым — ничего не теряется.

**Codex** подменяет `~/.codex/auth.json` (файлами, обратимо). **Claude** копирует между записями Keychain (keychain→keychain, токены не пишутся на диск) и обновляет e-mail в `~/.claude.json`; переключение один раз спросит доступ к Keychain. Важно: активную запись Claude читает и любая запущенная сессия Claude Code, поэтому не переключай аккаунт, которым пользуешься в живой сессии — если процессы `claude` запущены, диалог покажет их число и по умолчанию выберет «Отмену».

## Несколько аккаунтов

Меню → **«Добавить аккаунт Claude…»** или **«Добавить аккаунт Codex…»**. Диалог просит короткое имя профиля и сразу показывает результат (`Папка: ~/.claude-profiles/work`); кнопка «Создать» неактивна, пока имя не годится. Панель создаст `~/.claude-profiles/<имя>` (или `~/.codex-profiles/<имя>`), положит туда `login.command` и откроет его в Terminal.app. Пароли панель не спрашивает и не видит — вход делает CLI. Новые профили подхватываются автоматически, без перезапуска. Профиль, у которого вход не завершён, виден в **«Удалить аккаунт»** как «… — вход не выполнен» — так можно удалить и ошибочно созданный.
...
User-Agent: claude-code/<версия CLI из пути установки>
```

Родной `User-Agent` важен — без него эндпоинт заметно чаще отвечает 429. Любое поле-объект с числовым `utilization` считается окном, поэтому новые окна появляются сами, включая внутренние кодовые имена моделей. Инертные окна — `0 %` **и** без даты сброса, как заглушка `nimbus_quill`, которую эндпоинт отдаёт про запас, — скрываются, пока не оживут: незачем занимать место строкой, с которой ничего не сделать. Названия: `five_hour`/`seven_day` — понятные подписи; известные префиксы разворачиваются (`seven_day_opus` → «Недельное окно · Opus», показывается на планах с Opus); остальное показывается словами, а не сырым snake_case. `extra_usage` (доплата за расход сверх плана) в окна не попадает — это деньги, а не доля квоты — и уходит в строку статистики, но **только пока `is_enabled: true`**: тогда `used_credits` — живой счётчик потраченного (показывается как сумма: значение минорное, `10308` при `decimal_places: 2` — это `103.08 $`, плюс месячный потолок `monthly_limit`, если задан). Когда доплата выключена (`out_of_credits`), то же число — просто исторический остаток израсходованных кредитов и выглядело бы как активный счёт, поэтому строка скрывается.

Подзаголовок — `план · почта`, как у Codex. План (`Pro`, `Max`) — из записи Keychain; почты нет ни в одном эндпоинте, поэтому читаем её прямо из конфига CLI — `~/.claude.json` (или `<CLAUDE_CONFIG_DIR>/.claude.json` для доп. профилей), поле `oauthAccount.emailAddress`. Это обычное чтение файла, без Keychain и подпроцессов, поэтому диалог пароля не всплывает.

### Обновление токена

Токен доступа Claude живёт несколько часов. Если CLI не запускать, он протухает и колонка замирает — поэтому, когда токен просрочен, а в записи Keychain лежит живой `refreshToken`, панель продлевает его сама, ровно как это делает CLI:
...
{"grant_type": "refresh_token", "refresh_token": "…",
 "client_id": "9d1c250a-e61b-44d9-88ed-5944d1962f5e", "scope": "<права из записи>"}
```

`client_id` — публичный идентификатор клиента Claude Code, секрета в нём нет. Права берутся из самой записи, а не зашиты в код, чтобы набор не сузился молча.

Три неочевидные детали:

- **Refresh-токен ротируется.** Когда в ответе приходит новый, старый тут же перестаёт работать, поэтому результат записывается обратно в Keychain: без записи сломался бы вход самого CLI. Запись накладывается на *свежепрочитанную* запись, сохраняя `subscriptionType`, `rateLimitTier` и все поля, о которых панель не знает.
- **В Keychain пишем, только если refresh-токен реально сменился.** Если сервер не выдал новый refresh-токен — старый ещё годен, и мы ничего не пишем, а новый access-токен просто живёт в памяти до следующего обновления. Это важно: каждая запись в Keychain вызывает диалог авторизации macOS, а отказ от лишних записей делает его редким (разреши один раз кнопкой **«Разрешать всегда»**).
- **401 помечает именно этот токен как отвергнутый**, чтобы следующий заход обновлял его, а не перечитывал из Keychain тот же мёртвый. Если в записи оказался другой токен — значит, CLI обновился первым, и берётся он.

Продление происходит только по факту протухания, никогда заранее, — так окно для гонки с CLI остаётся маленьким.
...
Рисуются `rate_limit.primary_window`, `secondary_window` и все `additional_rate_limits[]` — последние с префиксом из `limit_name` либо `metered_feature`. Эти доп. записи — отдельные лимиты по конкретным моделям, которые API отдаёт помимо основной квоты (например `GPT-5.3-Codex-Spark` — отдельный тир модели); приложение показывает те имена, что пришли, так что список следует за OpenAI без правок кода. Название окна выводится из `limit_window_seconds` (`18000` → 5-часовое, `604800` → недельное). Подзаголовок — `plan_type · почта`; `credits.balance` и `rate_limit_reset_credits.available_count` становятся строками статистики (число доступных «полных сбросов» — как в самом приложении ChatGPT; `applicable_available_count` не используем, он равен 0 даже когда все кредиты доступны).
...
Автоматически приложение ходит только в два usage-эндпоинта выше и в OAuth-эндпоинт Anthropic, когда токен доступа Claude протух (см. [Обновление токена](#обновление-токена)). Другие запросы — только **вручную**: **Проверить наличие обновлений** дёргает API релизов GitHub (`api.github.com`) по клику (сам никогда), а если выбрать **Обновить сейчас** — скачивает `.zip` и `SHA256SUMS.txt` этого релиза с GitHub и ставит, только если сумма совпала. Сборки из CI подписаны ad-hoc, поэтому обновление меняет подпись, и macOS один раз переспросит доступ к Keychain (разовое **«Разрешать всегда»**); Developer ID подпись в CI убрала бы и это. Токены не логируются и не пишутся в обычные файлы; единственное, что записывается, — обновлённый токен Claude, обратно в ту же запись Keychain, которой владеет CLI. Бинарь `claude` не запускается вовсе (его запуск заставляет CLI читать креды через `/usr/bin/security` — а это диалог пароля Keychain): версию для User-Agent берём из пути установки, почту — из конфига CLI `~/.claude.json`. Запускается только `codex --version`, а Codex держит авторизацию в файле, не в Keychain. На диск идут только настройки и последние проценты со временем сброса — в `UserDefaults`, без единого токена. `URLSession` работает в ephemeral-режиме, без кук и дискового кэша.
...
[`.github/workflows/release.yml`](.github/workflows/release.yml) — по тегу `v*` или вручную с версией: собирает, гоняет самопроверку, сверяет версию в `Info.plist` с тегом, пакует через `ditto` (обычный `zip` портит подпись), считает SHA-256 собирает DMG через `scripts/build_dmg.sh` и публикует релиз с `.dmg` (ручная установка), `.zip` (по нему обновляется приложение) и `SHA256SUMS.txt` на оба файла. Заметки к релизу собираются автоматически: раздел версии из `CHANGELOG.md`, установка (сначала DMG, потом zip, включая обязательное снятие карантина для ad-hoc), коммиты с прошлого тега, таблица состава и контрольная сумма.
...
Sources/NotchLimits/
  ...
  Core/       планировщик опроса, кэш, история + прогноз, уведомления, хоткеи, добавление аккаунтов
Tools/
